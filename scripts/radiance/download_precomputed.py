...
def _files_byte_identical(left: Path, right: Path) -> bool:
    ...
    with left.open("rb") as left_handle, right.open("rb") as right_handle:
        while True:
            left_chunk = left_handle.read(DOWNLOAD_CHUNK_BYTES)
            right_chunk = right_handle.read(DOWNLOAD_CHUNK_BYTES)
...
def _existing_staged_file_conflicts(
    stage: Path, dest: Path, files: list[Path], force: bool
) -> list[Path]:
    if force:
        return []
    conflicts: list[Path] = []
    for rel_path in files:
        ...
        if not target.exists():
            continue
        if target.is_file() and _files_byte_identical(source, target):
            continue
        conflicts.append(target)
        if len(conflicts) >= 8:
            break
...
def _install_staged_files(stage: Path, dest: Path, force: bool) -> None:
    ...
    conflicts = _existing_staged_file_conflicts(stage, dest, files, force)
    if conflicts:
        rendered = "\n".join(f"  - {path}" for path in conflicts)
        raise RuntimeError(f"Archive would overwrite existing files. Re-run with --force to replace them:\n{rendered}")
    installed: list[Path] = []
    ...
    try:
        ...
        for rel_path in files:
            source = stage / rel_path
            target = safe_target(dest, str(rel_path))
            if target.exists() and target.is_file() and _files_byte_identical(source, target):
                continue
